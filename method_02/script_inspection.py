"""CLI de inspecao do Metodo 02 no Neo4j."""
...
from method_02.neo4j_store import get_driver, inspect_counts
from method_02.settings import configure_method_02_environment, get_neo4j_settings


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Inspeciona dados do Metodo 02 no Neo4j.")
    parser.parse_args(argv)

    configure_method_02_environment("method_02.script_inspection")
    ...
    try:
        ...
        print("Resumo Neo4j Method 02")
        print(f"- Neo4j: {settings.uri} database={settings.database}")
        print(f"- Documentos: {counts['document_count']}")
        print("- Nos:")
